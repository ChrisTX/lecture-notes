...
\begin{IEEEeqnarray*}{rCl}
	...
	&\overset{\text{\cref{thm:10}}}{=}& \lambda_n^y \lambda_n^x K(y, x) + \lambda_m^y \lambda_n^x K(y, x) - 2 \lambda_n^y \lambda_m^x K(y, x) \\
...
\end{theorem}
\begin{proof}
If $f(X) = \{ 0 \}$ then $f \in \spH_X^\perp$ by (*RE) % TODO
and conversely.
\end{proof}
...
\addtocounter{equation}{2} % 18 and 19 follow next lecture
...
\end{theorem}
\begin{proof}
This follows because any kernel matrix on a finite set $X$ is a Gram matrix for the functionals $\delta_{x_j}$ or the functions $K(x_j, \cdot)$, due to (*RE). % TODO
\end{proof}
\begin{remark}

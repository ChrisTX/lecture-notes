...
\begin{figure}
\centering
\begin{tikzpicture}[scale=\textwidth/8.5cm]
	\draw[->] (-4.2, 0) -- (4.2, 0);
	\draw[->] (0,0) -- (0, 1.7);
	\foreach \a in {2, 4, ..., 10}
		\draw[domain=-4:4, color=green!\a 0!red, thick] plot (\x, {exp(-\a*0.01*\x*\x)});
\end{tikzpicture}
...
\begin{tikzpicture}[scale=\textwidth/8.5cm]
	...
	\draw[->] (0,0) -- (0, 1.7);
	\foreach \a in {2, 4, ..., 10}
		\draw[domain=-4:4, color=green!\a 0!red, thick] plot (\x, {pow(1 + \x*\x, \a/30)});
\end{tikzpicture}
\captionsetup{labelformat=empty}

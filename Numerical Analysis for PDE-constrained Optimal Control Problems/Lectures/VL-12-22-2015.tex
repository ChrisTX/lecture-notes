...
&=& \sum{i=1}^m \left[ \left( y_i^h (\bar{x}_j ^h) - y_i(\bar{x}_j^h) \right) + y_i (\bar{x}_j^h) \right]
\end{IEEEeqnarray*}
This yields:
\begin{IEEEeqnarray*}{rCl}
...
Rearranging:
\begin{IEEEeqnarray*}{rCl}
ch^2 |\log h| &\geq& \sum_{i=1}^m \left[ \left( \bar{u}_i - \bar{u}_i^h \right) y_i (\bar{x}_j) + \bar{u}_i^h \underbrace{ \left( y_i(\bar{x}_j) - y_i(\bar{x}_j^h) \right) }_{\text{Taylor}} \right] \\

...
\begin{enumerate}[(i)]
\item $\Gamma_0$ has positive measure in $\Gamma$,
\item $\Gamma_1 = \Gamma$ and $\int_\Omega c_0^2 \dx + \int_\Gamma \alpha^2 \ds > 0$,
\end{enumerate}
then there exists exactly one weak solution $y$ and
\[
	\| y \|_{H^1} \leq c_{A} (\| f \|_{L^2(\Omega)} + \| g \|_{L^2(\Gamma_1)}).
\]
\end{theorem}
\begin{proof}
Exercise!
...
\begin{IEEEeqnarray*}{rCl"l}
Ay + c_0 y &=& f & \text{on } \Omega, \\
\partial_{\nu_A} y + \alpha y &=& g & \text{on } \Gamma.
\end{IEEEeqnarray*}
...
\begin{theorem}
Let $\Omega$ be of class $C^{m-1,1}$. Then, for $mp < N$ th trace operator $\tau$ is continuous from $W^{m,p}(\Omega) \to L^r(\Gamma)$, if $1 \leq  r \leq \frac{(N-1)p}{N - mp}$. For $mp = N$, $\tau$ is continuous for every $1 \leq r < \infty$.
\end{theorem}
\begin{proof}
See Adams ``Sobolev spaces''.
\end{proof}
...
minimize & J(y, u) &=& \IEEEeqnarraymulticol{2}{c}{ \frac{1}{2} \| y - y_\Omega \|_{L^2(\Omega)}^2 + \frac{\lambda}{2} \| u \|_{L^2(\Omega)}^2 } \\
subject to & - \nabla y &=& \beta u & \text{in } \Omega \\
& y &=& 0 & \text{on } \Gamma \\
and & u_a(x) &\leq& u(x) \leq u_b(x) & \text{\ac{ae} in } \Omega.
\end{IEEEeqnarray*}
\end{problem}
The set
\[
...
\begin{IEEEeqnarray*}{u"rCl"l}
minimize & J(y, u, v) &\coloneqq&  \IEEEeqnarraymulticol{2}{l}{ \frac{\lambda_\Omega}{2} \| y - y_\Omega \|_{L^2(\Omega)}^2 + \frac{\lambda_\Gamma}{2} \| y - y_\Gamma \|_{L^2}^2 } \\
&&& \IEEEeqnarraymulticol{2}{l}{\;\; {} + \frac{\lambda u}{2} \| u \|_{L^2(\Omega)} + \frac{\lambda v}{2} \| v \|_{L^2(\Omega)}^2 } \\
subject to & Ay + c_0 y &=& \beta_\Omega u & \text{in $\Omega$}, \\
& \partial_{\nu_A} y + \alpha y &=& \beta_\Gamma u & \text{on $\Gamma_1$}, \\
&  y &=& 0 & \text{on $\Gamma_0$}, \\
and & u_a(x) &\leq& u(x) \leq u_b(x) & \text{\ac{ae}\ in $\Omega$}, \\
& v_a(x) &\leq& v(x) \leq v_b(x) & \text{\ac{ae}\ on $\Gamma_1$}.
\end{IEEEeqnarray*}
Here, let $A, c_0, \alpha$ be as before, $\beta_\Omega \in L^\infty(\Omega)$, $\beta_\Gamma \in L^\infty(\Gamma_1)$, $u_a, u_b \in L^2(\Omega)$, $v_a, v_b \in L^2(\Gamma)$.
Assume (i) or (ii) hold in \cref{thm:c2e14}.
\end{problem}
...
&&& \;\; {} + \frac{\lambda u}{2} \| u \|^2 + \frac{\lambda v}{2} \| v \|_{L^2}^2 \\

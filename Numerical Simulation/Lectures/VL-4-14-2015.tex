...
\label{sec:c1}
\section{Existence results} % Section 1.2
\label{sec:c1e2}
...
\begin{remarknonumb}[Sublinear functional]
	Let $p\in X^*$, so $p:X\to\R$. $p$ is called a \emph{sublinear functional}, if
	\begin{IEEEeqnarray*}{rCl"l}
		p(\xi + \eta) & \leq & p(\xi) + p(\eta),&  \forall \xi,\eta\in X\\
		p(\alpha\xi) &=& \alpha p(\xi), & \text{for some }\alpha \geq 0 
	\end{IEEEeqnarray*}
\end{remarknonumb}
...
\begin{remarknonumb}
	The Minkowski functional can be seen as a generalization to the norm on the space. In general, it can be built for each \emph{absorbing subset} $A$ of $X$, i.e.
	\[
		\forall x\in X \; \exists r\geq 0\;:\;\alpha x\in A,\quad\forall\|\alpha\|\leq r.
	\]
\end{remarknonumb}
...
\end{theorem}
\begin{proof}
Since $X \setminus A$ is open, there exists an open ball $B$ around $\xi$ \ac{st} $A \cap B = \emptyset$. Since $B^\circ \neq \emptyset$, we can apply \cref{thm:c1e7} to obtain $\ell \in X^*$ \ac{st}
\[
...
\label{cor:c1e10}
Let $X$ be a normed space and $(\xi_n) \subseteq X, \xi_n \rightharpoonup \xi$. Then $\xi \in \overline{\conv\{ \xi_n \; : \; n \in \N \}}$.
\end{corollary}
...
\end{proof}
\begin{definitionnonumb}[coercive function]

...
Recall that a Banach space $U$ is called \emph{reflexive} if the canonical map $U \to U^{**}$, $u \mapsto u''$, $u''(\ell) = \ell(u)$ is surjective (in which case it is an isometric isomorphism, because the canonical embedding of $U$ in $U**$ is always linear and injective).
\begin{example}
\begin{itemize}
\item Hilbert spaces, 

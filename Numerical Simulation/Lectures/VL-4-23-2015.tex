...
\mathcal{L}_u(u_0, \lambda_0, \lambda_1^*) = \lambda_0 f'(u_0) + g'(u_0)^* \lambda_1^* &=& 0 & \text{in } U^*, \\
\noalign{that is \vspace{\jot}} \lambda_0 f'(u_0) h + \lambda_1^* (g'(u_0)h) &=& 0 & \forall h \in U.
\end{IEEEeqnarray*}
...
\end{theorem}
\begin{proof}
We first consider the case that $u_0$ is a regular point, then $\mathcal{R}(g'(u_0)) = V$. In this case it holds with $M = \{ u \in U \, : \, g(u) = 0\}$ that $T_{u_0} M = \mathcal{N}(g'(u_0))$ by \cref{thm:c1e31}. Let $h \in \mathcal{N}(g'(u_0))$. Since $T_{u_0} M$ is a linear space, there exists an admissible curve $\gamma : (- \varepsilon, \varepsilon) \to M$ \ac{st} $\dot\gamma(0) = h$. Then the function $t \mapsto f(\gamma(t))$ attains a local minimum at $t=0$. By the chain rule,
\[
...
\begin{enumerate}[(i)]
\item\label[equation]{thm:c1e36-i} $K^+$ is closed, convex and nonempty.
\item\label[equation]{thm:c1e36-ii} If $K \neq \emptyset$, then
\[
...
\end{proposition}
\begin{proof}
\labelcref{thm:c1e36-i} is trivial.

\labelcref{thm:c1e36-ii}: If $\ell \in K^+$, then
\[
...
\]
By \cref{thm:c1e36}, \labelcref{thm:c1e36-ii}, thus implies $\ell \in K^+$ and $-\ell \in K_{m +1}^+$. We choose $\ell_{m + 1} = -\ell$, $\ell_{m+2} = \ldots = \ell_{n+1} = 0$. Then,
\[
...
\end{equation}
which then proves \labelcref{thm:c1e36-ii}, since then $\ell = \ell_0 + \ldots + \ell_m$ for some $\ell_i \in K_i^+$, $i = 0, \ldots, m$.
...
\]
By \cref{thm:c1e34}, we can extend $F$ to $X$ \ac{st}

...
\begin{enumerate}[(i)]
\item \label[equation]{thm:c1e17-i} $C$ is bounded and $a$ is positive
\item \label[equation]{thm:c1e17-ii} $a$ is strongly positive (coercive)
\end{enumerate}
In case \labelcref{thm:c1e17-ii} the solution is unique. In case \labelcref{thm:c1e17-i} the solution is unique if $a$ is strictly positive.
\end{proposition}
\begin{proof}
...
\end{enumerate}
In cases \labelcref{thm:c1e18-ii}, \labelcref{thm:c1e18-iii} the solution is unique. In \labelcref{thm:c1e18-i} with $\lambda = 0$, the solution is unique if and only if $S$ is injective.
\end{theorem}
\begin{proof}
...
\]
Almost everything follows from \cref{thm:c1e17}. Uniqueness in case \labelcref{thm:c1e18-i} with $\lambda = 0$: If $S$ is injective, then $a$ is strictly positive. If $S$ is not injective, then $f(u) = f(u + h)$ for all $h$ from the null space of $S$.
...
\label{ex:c1e21}
\begin{enumerate}[(i)]
\item\label[equation]{ex:c1e21-i} Let $A : U \to V$ be bounded and linear. Then $A$ is Fréchet-differentiable, and $A'(u) = A \quad \forall u \in U$.
\item\label[equation]{ex:c1e21-ii} \underline{Quadratic function on a Hilbert space:} \\
...
\[
	\nabla f(u) = Au - b.
\]
\item\label[equation]{ex:c1e21-iii} Let $U = C[0, 1]$ and $f(u) = \sin(u(1))$.
...
\end{equation}
\end{mdframed}
then $f'(\bar{u}) = 0$. If $C$ is convex and $f$ is convex on $C$, $f'(\bar{u}) = 0$ implies that $\bar{u}$ solves \labelcref{eq:thm-c1e23-star}.
\end{theorem}

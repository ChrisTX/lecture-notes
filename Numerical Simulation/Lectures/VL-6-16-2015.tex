...
\addtocounter{dummythm}{2} % 7 and 8 have not been in the lecture
\begin{definition}[classes of vector-valued functions] % Def 3.9
\label{def:c3e9}
...
\end{enumerate}
\end{definition}
\begin{definition}[space \texorpdfstring{$L^p(0, T; X)$}{Lp(0, T; X)}] % Def 3.10
\label{def:c3e10}
...
\begin{proposition}[Existence] % Prop 3.15
...
\begin{propdef} % Prop & Def 3.16
\label{thm:c3e16}
Let ``$V \subseteq H \subseteq V^*$'' be a Gelfand triple and let $1 < p < \infty$, $\frac{1}{p} + \frac{1}{q} = 1$, $0 < T < \infty$. The following hold:
\begin{enumerate}[(i)]
\item\label[equation]{thm:c3e16-i} The set of all $y \in L^p(0, T; V)$ that have a generalized derivative
\[
...
\item\label[equation]{thm:c3e16-ii} The embedding
\[
	W^{1, p}(0, T; V, H) \subseteq C([0, T]; H)
\]
is continuous.
\item\label[equation]{thm:c3e16-iii} For all $y, v \in W^{1, p}(0, T; V, H)$ and arbitrary $t, s, 0 \leq s \leq t \leq T$ it holds
\[
	\langle y(t), v(t) \rangle_H - \langle y(s), v(s) \rangle_H = \int_s^t \langle y'(\tau), v(\tau) \rangle_V + \langle v'(\tau), y(\tau) \rangle_V \: \mathrm{d} \tau
\]
(\emph{integration by parts}). The point evaluations on the left side make sense by \labelcref{thm:c3e16-ii}.
\end{enumerate}
\end{propdef}
